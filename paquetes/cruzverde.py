...
url_principal = 'https://www.cruzverde.com.co'
...
def filtra_links(X):
    return (sum([patron in X for patron in patrones_link])>=1)&('http://blog.cruzverde.com.co' not in X)
...
with closing(Chrome(executable_path = chromedriver, options=options)) as navegador:
    ...
    with open('./web_scraping/data/cat_urls_cruzverde.csv', 'w+') as f:
        f.write('\n'.join(cat_urls))
...
with open('./web_scraping/data/cat_urls_cruzverde.csv', 'r') as f:
    cat_urls = f.read()
